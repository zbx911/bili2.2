...
class BiliMainReq:
    @staticmethod
    async def send_coin2audio(user, sid, num_sent):
    ...
    async def send_coin2video(user, aid, num_sent):
        ...
        pcheaders = {
            **(user.dict_bili['pcheaders']),
            'referer': f'https://www.bilibili.com/video/{aid[1]}'
            }
        data = {
            'aid': aid[0],
            'multiply': num_sent,
    ...
    async def share_video(user, aid, bvid):
        ...
        data = {'aid': aid, 'csrf': user.dict_bili['csrf']}
        json_rsp = await user.other_session.request_json('POST', url, data=data, headers=user.dict_bili['pcheaders'])
        return json_rsp
    ...
    @staticmethod
    async def fetch_uper_audios(user, uid):
        url = f'https://api.bilibili.com/audio/music-service/web/song/upper?' \
              f'uid={uid}&pn=1&ps=30&order=1&jsonp=jsonp'
        json_rsp = await user.other_session.request_json('GET', url)
        return json_rsp
